...
const QuizManager = {
  ...
  // ✅ Doğru cevap index'ini güvenli şekilde bulur.
  // Destekler:
  // a: 0
  // a: "A"
  // a: "Doğru cevap metni"
  getCorrectIndex(question) {
    ...
    // En doğru format: a = 0,1,2,3,4
    if (typeof answer === 'number' && Number.isInteger(answer)) {
      return answer >= 0 && answer < question.o.length ? answer : -1;
    }

    // Eski/AI format ihtimaline karşı: a = "A", "B", "C", "D", "E"
    if (typeof answer === 'string') {
      ...
      if (Object.prototype.hasOwnProperty.call(letterMap, upper)) {
        ...
        return idx < question.o.length ? idx : -1;
      }
      ...
      // a = "Doğru şık metni" gibi gelirse
      const exactIndex = question.o.findIndex(opt => String(opt) === trimmed);
      if (exactIndex !== -1) return exactIndex;

      // Küçük/büyük harf ve boşluk farkı varsa
      const normalizedAnswer = trimmed.toLowerCase();
      const looseIndex = question.o.findIndex(opt => String(opt).trim().toLowerCase() === normalizedAnswer);
  ...
  loadAIGeneratedTest() {
    try {
      ...
      if (!testData.questions || !Array.isArray(testData.questions)) {
        console.warn('⚠️ AI testi geçersiz soru formatında');
        return null;
      }
    ...
    try {
      ...
      if (useAI && aiTest && aiTest.questions && aiTest.questions.length > 0) {
        ...
        this.state = {
          ...
          elapsedSeconds: 0,
          isReviewing: false,
          testTitle: aiTest.title || 'AI Testi',
          testDescription: aiTest.description || null,
          eventListenersAttached: this.state.eventListenersAttached
        };

        this.state.answers = new Array(aiTest.questions.length).fill(null);

        Utils.showToast(`🤖 AI Testi: ${this.state.testTitle} - ${aiTest.questions.length} soru`, 'info', 4000);
      } else {
  ...
  displayOptions(question) {
    ...
    if (!question || !Array.isArray(question.o)) {
      optionsList.innerHTML = '<div class="question-explanation">Bu soru geçersiz formatta.</div>';
      return;
    }
    ...
    question.o.forEach((option, index) => {
      ...
      optionDiv.innerHTML = `
        ...
        <span>${Utils.sanitizeHTML(String(option))}</span>
      `;
    ...
    if (this.state.isReviewing && question.explanation) {
      ...
      explanationDiv.innerHTML = `
        ...
        <p>${Utils.sanitizeHTML(String(question.explanation))}</p>
      `;
  ...
  selectOption(index) {
    ...
    try {
      ...
      if (correctIndex === -1) {
        console.error('❌ Doğru cevap index’i bulunamadı:', question);
        Utils.showToast('Bu sorunun doğru cevap formatı hatalı!', 'error');
        return;
      }
    ...
    explanationDiv.innerHTML = `
      ...
      <div style="display:flex; align-items:flex-start; gap:8px; margin-top:10px;">
        ...
        <div>
          ...
          <p style="color: var(--text-secondary); line-height:1.6; margin:5px 0 0;">${Utils.sanitizeHTML(String(question.explanation))}</p>
        </div>
      </div>
    `;
